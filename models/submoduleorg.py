...
class feature_extraction(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            ...
            conv2d_bn(in_channels=3, out_channels=1, kernel_size=3, stride=2, padding=1),
            nn.MaxPool2d(kernel_size=2, stride=2),
            conv2d_bn(in_channels=1, out_channels=4, kernel_size=3, stride=1, padding=1),
            conv2d_bn(in_channels=4, out_channels=2, kernel_size=3, stride=1, padding=1)
        )
        # H/4 x W/4 -> H/8 x W/8
        self.conv2 = nn.Sequential(
            nn.MaxPool2d(kernel_size=2, stride=2),
            conv2d_bn(in_channels=2, out_channels=8, kernel_size=3, stride=1, padding=1),
            conv2d_bn(in_channels=8, out_channels=4, kernel_size=3, stride=1, padding=1)
        )
        ...
        self.conv4 = nn.Sequential(
            conv2d_bn(in_channels=8+4, out_channels=8, kernel_size=3, stride=1, padding=1),
            conv2d_bn(in_channels=8, out_channels=4, kernel_size=3, stride=1, padding=1)
        )
    ...
    def forward(self, x):
        ...
        upconv3 = F.interpolate(input=conv3, scale_factor=2.0, mode='nearest')
        conv4 = self.conv4(torch.cat([conv2, upconv3], dim=1))
        # H/8 x W/8 -> H/4 x W/4
        upconv4 = F.interpolate(input=conv4, scale_factor=2.0, mode='nearest')
