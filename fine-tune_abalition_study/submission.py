...
def main():
    ...
    for inx in index_pbar:
        ...
        # img = (img/255).astype('float32') #cv2 needs 
        # img_log_scale = np.log10(img)
        # img_normalized = (img_log_scale - np.min(img_log_scale)) / (np.max(img_log_scale) - np.min(img_log_scale))
        # img_min = np.min(img_normalized)
        # img_max = np.max(img_normalized)
        # img_normalized_stretched = (img_normalized - img_min) * (1 / (img_max - img_min))

        # rgb_image = cv2.cvtColor(img,cv2.COLOR_GRAY2RGB)
        # rgb_image_np = np.array(rgb_image)
        # rgb_image = cv2.applyColorMap((img_normalized*255).astype(np.uint8), cv2.COLORMAP_JET)
        # rgb_image_np = np.array(rgb_image)
        # rgb_image = cv2.applyColorMap((img_normalized_stretched*255).astype(np.uint8), cv2.COLORMAP_JET)
        # rgb_image_np = np.array(rgb_image)
        img = (img/255).astype('float32') #cv2 needs 
        # img_log_scale = np.log10(img)
        img_log_scale = np.log(img + 1) / np.log(200 + 1)
        img_normalized = (img_log_scale - np.min(img_log_scale)) / (np.max(img_log_scale) - np.min(img_log_scale))
        # rgb_image = cv2.cvtColor(img,cv2.COLOR_GRAY2RGB)
        # rgb_image_np = np.array(rgb_image)
        rgb_image = cv2.applyColorMap((img_normalized*255).astype(np.uint8), cv2.COLORMAP_JET)
        ...
        cv2.imwrite(os.path.join(args.output_dir, test_left_img[inx].split('/')[-1]),rgb_image_np)
