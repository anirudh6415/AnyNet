...
parser.add_argument('--loss_weights', type=float, nargs='+', default=[0.5, 0.5, 0.5, 0.5])
...
#                     help='learning rate')
parser.add_argument('--lr', type=float, default=5e-3,#pir
                    help='learning rate')
...
def main():
    global args
    df_loss = pd.DataFrame(index=range(1,args.epochs),columns=('epoch', 'stage1', 'stage2','stage3','stage4'))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(TrainImgLoader, model, optimizer,df_loss, log, epoch)
    ...
    test(TestImgLoader, model, log,df_mean,df_std)
#     potting of mean and sd and loss
    print(df_mean)
    print(df_std)
    print(df_loss)
    writer = pd.ExcelWriter('output1.xlsx')
    df_mean.to_excel(writer, index=False)
    writer.save()
    ...
    # plt.plot(df_loss['epoch'],df_loss['stage1'],marker='o',color='blue')
    # plt.plot(df_loss['epoch'],df_loss['stage2'],marker='o',color='teal',linestyle='dashed')
    # plt.plot(df_loss['epoch'],df_loss['stage3'],marker='o',color='red')
    plt.plot(df_loss['epoch'],df_loss['stage4'],marker='o',color='orange',linestyle='dashed')
    plt.xlabel(r'epoch')
    plt.ylabel(r'Loss')
    plt.legend(['stage4'])
...
def train(dataloader, model, optimizer,df_loss, log, epoch=0):
    ...
    df_loss.loc[epoch] = [epoch,stage1,stage2,stage3,stage4]
